function  plot_part_bif_r(data,ind_x,ind_y,...
...
for i = 1:(size(ind,1)-1)
    if data(ind(i)+1,ind_stab)==6
       plot(data(ind(i):ind(i+1),ind_x),data(ind(i):ind(i+1),ind_y),'r',...
           'linewidth',linewidth) 
    ...
    elseif data(ind(i)+1,ind_stab)==1
        plot(data(ind(i):ind(i+1),ind_x),data(ind(i):ind(i+1),ind_y),...
            'r--','color',[1    0.6100    0.1700],'linewidth',linewidth)
    else
        plot(data(ind(i):ind(i+1),ind_x),data(ind(i):ind(i+1),ind_y),...
            'r--','linewidth',linewidth)
    end
...
for i = 1:(size(ind,1)-1)
    if data(ind(i)+1,ind_stab)==6
       plot(data(ind(i)+1:ind(i+1),ind_x),data(ind(i)+1:ind(i+1),ind_y),'r',...
           'linewidth',linewidth) 
    elseif data(ind(i)+1,ind_stab)==2
        plot(data(ind(i)+1:ind(i+1),ind_x),data(ind(i)+1:ind(i+1),ind_y),...
            'r--','color',[1    0.8100    0.3700],'linewidth',linewidth) 
    elseif data(ind(i)+1,ind_stab)==1
        plot(data(ind(i)+1:ind(i+1),ind_x),data(ind(i)+1:ind(i+1),ind_y),...
            'r--','linewidth',linewidth)
    else
