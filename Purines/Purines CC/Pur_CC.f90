...
!--------------------------------------------------------------------------------------
        ...
        DOUBLE PRECISION delta, gamma, ct, ce, tau_c
        DOUBLE PRECISION V_pkc, Kplc, Kpkc, V_plc
        DOUBLE PRECISION kL, k_L, k_r, kmin, k_i
        DOUBLE PRECISION L_t, Q_t, omega1, omega2
        ...
        ! ODE's
        WRITE(10,*) Kp_tilde
        F(1) = (1-R1)*rho1_d - R1*omega1*Q*rho1_p
        F(2) = (1-R2)*rho2_d - R2*omega2*Q*rho2_p
        F(3) = (Jipr - Jserca)/tau_c
        F(4) = (h_inf-h)/tau
        F(5) = kL*L - k_i*p 
