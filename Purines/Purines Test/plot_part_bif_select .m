function  plot_part_bif_select(data,ind_x,ind_y,ind_stab,threshold,ind_figure,...
    linewidth,iseq,data_x_min,data_x_max)

data = data(((data(:,ind_x)>data_x_min)&(data(:,ind_x)<data_x_max)),:);
...
for i = 1:(size(ind,1)-1)
    if data(ind(i)+1,ind_stab)==3
       plot(data(ind(i):ind(i+1),ind_x),data(ind(i):ind(i+1),ind_y),'b',...
           'linewidth',linewidth) 
    elseif data(ind(i)+1,ind_stab)==2
        plot(data(ind(i):ind(i+1),ind_x),data(ind(i):ind(i+1),ind_y),...
            '--','color',[1    0.8100    0.3700],'linewidth',linewidth) 
...
for i = 1:(size(ind,1)-1)
    if data(ind(i)+1,ind_stab)==3
       plot(data(ind(i)+1:ind(i+1),ind_x),data(ind(i)+1:ind(i+1),ind_y),'b',...
           'linewidth',linewidth) 
    elseif data(ind(i)+1,ind_stab)==2
